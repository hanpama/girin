...
pub fn render(outdir: &PathBuf, s: &Schema) -> Result<()> {
    render_directory(outdir.join("impl"), Traversal::new(s))?;
    Ok(())
}
        ...
        match def {
            Type::Definition(inner) => match inner {
                Definition::ObjectDefinition(inner) => {
                    render_object_config(&mut src, d.schema, inner);
                    src.line("");
                    src.line("");
                }
                Definition::InterfaceDefinition(inner) => {
                    render_interface_config(&mut src, d.schema, inner);
                    src.line("");
                    src.line("");
                }
                Definition::ScalarDefinition(inner) => {
                    render_scalar_config(&mut src, d.schema, inner);
                    src.line("");
                    src.line("");
                }
                _ => { /* noop */ }
            ...
            Type::Extension(inner) => match inner {
                Extension::ObjectExtension(inner) => {
                    render_object_ext_config(&mut src, d.schema, inner);
                    src.line("");
                    src.line("");
                }
                Extension::InterfaceExtension(inner) => {
                    render_interface_ext_config(&mut src, d.schema, inner);
                    src.line("");
                    src.line("");
                }
                _ => { /* noop */ }
...
fn render_object_config(src: &mut SourceCode, s: &Schema, def: &ObjectDefinition) {
    let module_accessor = def.module.join(".");
...
fn render_interface_config(src: &mut SourceCode, s: &Schema, def: &InterfaceDefinition) {
    // def.module
    let module_accessor = def.module.join(".");
    let impl_name = naming::impl_type(&def.name);
    let spec_name = naming::spec_type(&def.name);
...
fn render_scalar_config(src: &mut SourceCode, s: &Schema, def: &ScalarDefinition) {
    let module_accessor = def.module.join(".");
    let impl_name = naming::impl_type(&def.name);
    let spec_name = naming::spec_type(&def.name);
...
fn render_object_ext_config(src: &mut SourceCode, s: &Schema, def: &ObjectExtension) {
    let module_accessor = def.module.join(".");
    let impl_name = naming::impl_type(&def.name);
    let spec_name = naming::spec_type(&def.name);
...
fn render_interface_ext_config(src: &mut SourceCode, s: &Schema, def: &InterfaceExtension) {
    let module_accessor = def.module.join(".");
    let impl_name = naming::impl_type(&def.name);
    let spec_name = naming::spec_type(&def.name);
...
fn render_field_resolver(src: &mut SourceCode, resolve: &Resolve) {
    src.import(&format!(
        "from {} import source",
        ".".repeat(resolve.field.module.len() + 1)
    ));
